...
export default function UndirectedGraphVisualizer({ input, op, opArgs, trigger }) {
  const [graph, setGraph] = useState({ nodes: [], edges: [], traversal: [] });

  useEffect(() => {
    setGraph(GR.create(parse(input), "undirected"));
  }, [input]);

  useEffect(() => {
    if (!op) return;
    setGraph(g => GR.apply(g, "undirected", op, opArgs));
  }, [trigger]);
  ...
  return (
    <svg width={700} height={450}>
      ...
      {graph.edges.map((e, i) => {
        ...
        if (!a || !b) return null;
        return <line key={i} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="white" />;
      })}
